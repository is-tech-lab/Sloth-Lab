...
**実行条件**: ブラウンフィールド (Brownfield) プロジェクトが検出された場合（ワークスペースに既存コードが見つかった場合）

**スキップ条件**: グリーンフィールド (Greenfield) プロジェクト（既存コードなし）

**再実行の動作**: 再実行は `workspace-detection.md` によって制御される。既存のリバースエンジニアリング成果物が見つかり、まだ最新の状態である場合、それらが読み込まれてリバースエンジニアリングはスキップされる。成果物が古い場合（コードベースの最終重大更新より古い場合）、またはユーザーが明示的に再実行を要求した場合、成果物が現在のコード状態を反映するようにリバースエンジニアリングが再実行される
...
- 設定ファイルを通じたパッケージの関係
- パッケージタイプ: アプリケーション (Application)、CDK/インフラ (CDK/Infrastructure)、モデル (Models)、クライアント (Clients)、テスト (Tests)
...
- **必須 (MANDATORY)**: ユーザーが明示的に承認するまで進まない
- **必須 (MANDATORY)**: ユーザーのレスポンスを完全な生入力とともに `audit.md` に記録する
